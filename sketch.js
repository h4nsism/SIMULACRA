...
function setup() {
  ...
  textFont(font)
    ...
    scripts[0][1] = "[융은 모든 사람에게 그림자가 있음을 강조했다.]";
    scripts[0][2] = "[실재하는 모든 것은 그림자를 드리우는데, 자아와 그림자의 관계는 빛과 그늘의 관계와 같으며,]";
    scripts[0][3] = "[이 그림자가 우리를 인간으로 만들어 준다.]";
    scripts[0][4] = "[그림자는 완전히 없앨 수 있는 것이 아니므로]";
    scripts[0][5] = "[최선의 방법은 그림자와 화해하는 것이다.]";
    scripts[0][6] = "[그림자를 없애려는 시도는 해서도 안 된다.]";
    scripts[0][7] = "...";
    scripts[0][8] = "[이 게임은 그림자를 다루는 시리어스게임입니다.]";
    scripts[0][9] = "[ASDF 키로 방향 조작,]";
    scripts[0][10] = "[스페이스 바로 대화/아이템 조작 등의 상호작용을 할 수 있습니다.]";
    scripts[0][11] = "...";
    scripts[0][12] = "천둥이 친다. 그림자가 너무 크다. 두렵다.";
    scripts[0][13] = "밖으로 나가자.";
    scripts[0][14] = "";
  scripts[1] = [];
...
function intro(){
  ...
  textSize(15);
  fill("#ede9dc");
  text("press SPACE to start", width/2, 370);
main = new Main();
shadow = new Shadow();
shadow.shadowSetup();
shadow.offset();
